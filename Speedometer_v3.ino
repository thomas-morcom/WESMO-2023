...
void drawSpeedNumbers(int x, int y, int speed) {
  ...
  // gfx->fillRect(x,y-h,w,h,PURPLE);
  // gfx->drawRect(x+w,y-h,60,h,YELLOW);
  // gfx->fillRect(x+w+s,y-h,w,h,ORANGE);
  // gfx->drawRect(x+w+s+w,y-h,60,h,YELLOW);
  // gfx->fillRect(x+w+s+w+s,y-h,w,h,RED);

  gfx->fillRect(x,y-h,(3*charWidth + 2*charSpace)*speedCharScale,h+speedCharScale,BACKGROUND);
  // gfx->fillRect(x,y-h,w,h+1,BACKGROUND);
  // gfx->fillRect(x+w+s,y-h,w,h+1,BACKGROUND);
  // gfx->fillRect(x+w+s+w+s,y-h,w,h+1,BACKGROUND);


  if ((prevSpeed > 99 && speed < 100) || (prevSpeed > 9 && speed < 10)) {
    // gfx->fillRect(x, y, (3*charWidth + 2*charSpace)*speedCharScale, charHeight*speedCharScale, BACKGROUND);
    // printf("Manually Blanking the speed display\n");
    // printf("BLANKING --> X: %d, Y: %d, Width: %d, Height: %d\n", x, y, blankWidth, blankHeight);

    // gfx->fillRect(x, y-h+51, (3*charWidth)*speedCharScale, charHeight*speedCharScale, BACKGROUND);
    gfx->fillRect(x, y, (3*charWidth + 2*charSpace)*speedCharScale, charHeight*speedCharScale+1, BACKGROUND);
  } 

  if (speed < 10) {
    numOfChars = 1;
    x += 2*charWidth*speedCharScale;
  } else if (speed < 100) {
    numOfChars = 2;
    x += charWidth*speedCharScale;
  }
  else {
    numOfChars = 3;
  }
  ...
  gfx->setCursor(x, y);
  // gfx->setFont(u8g2_font_inb63_mn);
  gfx->setFont(u8g2_font_logisoso62_tn);
  // gfx->setFont(u8g2_font_logisoso78_tn);
  ...
  // gfx->fillRect(x,y,(3*charWidth + 2*charSpace)*speedCharScale,speedCharScale,BACKGROUND);
...
void setup() {
  ...
  // speedCoordinates = {"KM/H",25,200,3*charWidth*speedCharScale,charHeight*speedCharScale, 0.0};
  ...
  // drawString(speedCoordinates.x+speedCoordinates.width-speedCharScale, speedCoordinates.y+speedCoordinates.height+10+speedCharScale, speedCoordinates.str);
  ...
  // drawString(speedCoordinates.x+speedCoordinates.width-speedCharScale, speedCoordinates.y+speedCoordinates.height+10+175+speedCharScale, speedCoordinates.str);
...
void loop() {
  ...
  // for (int i = 99; i > -1; i--){
    ...
    drawSpeedNumbers(speedCoordinates.x, speedCoordinates.y, i+50);
