...
void drawSpeedNumbers(int x, int y, int speed)
{
  ...
  // gfx->fillRect(x+s,y-h,(3*w + 3*s),h+speedCharScale,YELLOW);
  // gfx->fillRect(x+s+w+s+w+s,y-h,w+s,h+speedCharScale,PURPLE);
  // gfx->fillRect(x,y,1,100,RED);
  ...
  // gfx->fillRect(x,y-h,(3*w+3*s)+s,h+speedCharScale,RED);
...
void loop()
{
  for (int i = 0; i < 100; i++)
  {
    ...
    drawSpeedNumbers(speedCoordinates.x, speedCoordinates.y, i * 2);
